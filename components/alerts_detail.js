...
class AlertsDetail extends Component {
  render() {
    ...
    if (covid && tested) {
      notification = `A student you were last in contact with on ${getDateUSFormatString(contactDate)} has been tested (as previously alerted) and diagnosed as positive for COVID-19.`;
      ...
      notification = `Earlier, you were alerted that a student you were last in contact with on ${getDateUSFormatString(contactDate)} was being tested for COVID-19; we are pleased to report that the individual in question tested negative.`;
      recommendation = 'Please continue to monitor potential symptoms.';
    } else { // not tested/is being tested
      notification = `A student you were last in contact with ${getDateUSFormatString(contactDate)} is being tested for COVID-19.`;
